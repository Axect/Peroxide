...
#[derive(Debug, Clone)]
pub struct Uniform<T> where T: PartialOrd + SampleUniform + Copy{
    pub range: (T, T),
}

impl<T> Uniform<T> where T: PartialOrd + SampleUniform + Copy {
    pub fn new(start: T, end: T) -> Uniform<T> {
        Uniform {
            range: (start, end),
        }
    }
...
impl<T> RNG for Uniform<T> where T: convert::Into<f64> + SampleUniform + PartialOrd + Copy {
    fn sample(&self, n: usize) -> Vec<f64> {
