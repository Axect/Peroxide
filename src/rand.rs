...
impl Normal {
    ...
    /// use peroxide::*;
    ///
    /// let a = Normal::new(0, 1); // Normal Distribution with mean 0 & sd 1
    /// ```
    pub fn new<T, S>(m: T, s: S) -> Normal
        where T: convert::Into<f64>, S: convert::Into<f64> {
        Normal { m: m.into(), s: s.into() }
    }
}
...
impl RNG for Normal {
    ...
    fn sample(&self, n: usize) -> Vec<f64> {
        ...
        for i in 0 .. n {
            v[i] = ziggurat(&mut rng, self.s) + self.m;
        }
        v
    }
}
